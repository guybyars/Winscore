...
void CFlight::Serialize(CArchive & ar)
{
	...
	if (ar.IsStoring())
	{
		ar << m_strCID;
		ar << (WORD)m_eClass;
		ar << (WORD)m_bAllTurnpointsAcheived;
		ar << (WORD)m_bValidFinish;
		ar << (WORD)m_bValidStart;
		ar << (WORD)m_bLocked;
		ar << m_iStartPenalty;
		ar << m_iOptions;
		SerializeTime ( ar, m_cRollTime) ;
		SerializeTime ( ar, m_cStartTime);
		SerializeTime ( ar, m_cFinishTime);
		SerializeTime ( ar, m_cLandingTime);
		ar << (WORD)m_eStatus;
		ar << (WORD)m_eTaskType;
		ar << m_strLandoutAirfield;
		ar << m_iLandoutAirfield;


		ar << m_nAcheivedTurnpoints;
		for( int i=0; i<m_nAcheivedTurnpoints; i++ )
			ar << m_aiAcheivedTurnpoints[i];

		m_cStartGate.Serialize(ar);
		m_cFinishGate.Serialize(ar);

		ar << m_nTaskPoints;
		for( int i=0; i<MAXTASKTPS; i++ )
			ar << m_aiTaskTurnpointIDs[i];

		CStringArray strArray;
		GetWarningsArray(strArray);
		strArray.Serialize(ar);
//		m_strWarnings.Serialize(ar);
		m_cFinishLocation.Serialize(ar);
		m_cLandingLocation.Serialize(ar);
		m_cStartFix.Serialize(ar);
		m_cScoredLandingPointLocation.Serialize(ar);

		ar << m_nMATTaskPoints;
		ar << m_iSoughtTurnpoint;
		ar << m_dDistance;

		for( int i=0; i<m_nTaskPoints; i++ )
			{
			ar<<m_aiTurnAreaTurnpointFixIDs[i];
			m_acTurnAreaTurnpointLocations[i].Serialize(ar);
			}

	    ar.SerializeClass(RUNTIME_CLASS(CIGCFile));
		CIGCFile::Serialize(ar);
		}
	else
		{
		...
		if( iSchema <=2 ) 
			...
			WORD w;
			ar >> m_strCID;
			ar >> w; m_eClass=(EClass)w;
			ar >> w; m_bAllTurnpointsAcheived=(w==0)?(false):(true);
			ar >> w; m_bValidFinish=(w==0)?(false):(true);
			ar >> w; m_bValidStart=(w==0)?(false):(true);
			ar >> w; m_bLocked=(w==0)?(false):(true);
			ar >> m_iStartPenalty;
			ar >> m_cRollTime;
			ar >> m_cStartTime;
			ar >> m_cFinishTime;
			ar >> m_cLandingTime;			
			ar >> w; m_eStatus=(EFlightStatus)w;
			ar >> m_strLandoutAirfield;
			ar >> m_iLandoutAirfield;

			ar >> m_nAcheivedTurnpoints;
			for( int i=0; i<m_nAcheivedTurnpoints; i++ ) ar >> m_aiAcheivedTurnpoints[i];

			ar >> iStartPointID;
			ar >> iFinishPointID;
			m_cStartGate.SetPointID(iStartPointID);
			m_cFinishGate.SetPointID(iFinishPointID);
			
			ar >> m_nTaskPoints;
			for( int  i=0; i<m_nTaskPoints; i++ ) ar >> m_aiTaskTurnpointIDs[i];

			CStringArray strWarnings;
			strWarnings.Serialize(ar);
			SetWarningsArray(strWarnings);
			...
			// Manually deserialize the IGC stuff in the old file
			ar >> m_strFilePath;
			ar >> w; m_bFileMissing=(w==0)?(false):(true);
			ar >> w; m_bValid=(w==0)?(false):(true);
			ar >> m_mtime;
			ar >> m_strErrorText;
			ar >> m_strFileName;
			ar >> m_strGliderType;
			ar >> m_strGliderID;
			ar >> m_strCompetitionID;
			ar >> m_strPilot;
			ar >> m_strSite;
			ar >> m_strUnits;
			ar >> m_strManufacturer;
			ar >> m_strFDRID;
			ar >> m_nPositionPoints;
			ar >> m_nLinesRead;
			ar >> m_iDay;
			ar >> m_iMonth;
			ar >> m_iYear;
			ar >> m_iAccuracyStartPos;
			ar >> m_iAccuracyEndPos;
			ar >> m_iENLStartPos;
			ar >> m_iENLEndPos;
			}
	  else if( iSchema==3 )
			...
			ar >> m_nTaskPoints;
			for( int i=0; i<MAXTASKTPS; i++ ) ar >> m_aiTaskTurnpointIDs[i];
...
//	else if( m_pDoc && ((CWinscoreDoc*)m_pDoc)->FailedSecurity(m_strFileName) )
//		SetOption(FLT_SECURITYFAILED);
	else if( IsCAI302File() )
		{
		char strResult[1024];
		memset(strResult,0,1024);

		int iRet=CAIValidateIGCFile( GetNonRelativePath(m_strFilePath),
									1024,
									strResult);
		if( iRet==CER_OK )
			{
		...
		if( CFile::GetStatus( strTemp, rStatus ) )
			...
			if( iRet==CER_OK )
				...
				{
				CString strWarn;
				strWarn.Format(_T("SECURITY FAILURE - .cai file found, but it failed manufacturers security check.") );
				AddWarning(eSecurity,0,strWarn);
				if( m_pDoc ) ((CWinscoreDoc*)m_pDoc)->LogFailed(m_strFileName);
				SetOption(FLT_SECURITYFAILED);
				}
			...
			{
			CString strWarn;
			strWarn.Format(_T("SECURITY NOT CHECKED - Could not find the .cai file generated by the GPS-NAV Recorder for this log.") );
			AddWarning(eSecurity,0,strWarn);
			if( m_pDoc ) ((CWinscoreDoc*)m_pDoc)->LogFailed(m_strFileName);
			SetOption(FLT_SECURITYFAILED);
			}
...
bool CFlight::CheckSecurity2(CString &cResult)
	...
	if( IsCAI302File() )
		...
		if( CFile::GetStatus( strTemp, rStatus ) )
			...
			{
			cResult.Format(_T("SECURITY NOT CHECKED - Could not find the .cai file generated by the GPS-NAV Recorder"));
			return false;
			}
	  }
	else
