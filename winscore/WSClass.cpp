...
void CWSClass::Serialize(CArchive & ar)
	{
	if (ar.IsStoring())
		{
		ar << m_strName;
		ar << m_iOptions;
		ar << m_iAirfieldBonusPoints;
		ar << m_dNoFinisherPoints_ASGNED;
		ar << m_dMDP_M_ASGNED;
		ar << m_dMDP_B_ASGNED;
		ar << m_dMSP_M_ASGNED;
		ar << m_dMSP_B_ASGNED;
		ar << m_dNoFinisherPoints_POST;
		ar << m_dMDP_M_POST;
		ar << m_dMDP_B_POST;
		ar << m_dMSP_M_POST;
		ar << m_dMSP_B_POST;
		...
		if( iSchema==2 ) 
			...
			{
			ar >> m_strName;
			ar >> w; SetHandicapped(		(w==0)?(false):(true));
			ar >> w; SetUseShortTaskFactor(	(w==0)?(false):(true));
			ar >> w; SetUseScoredCompletionRatio((w==0)?(false):(true));
			ar >> w; SetActive(				(w==0)?(false):(true));
			ar >> w; SetUsingWDSA(			(w==0)?(false):(true));
			ar >> m_iAirfieldBonusPoints;
			ar >> m_dNoFinisherPoints_ASGNED;
			ar >> m_dMDP_M_ASGNED;
			ar >> m_dMDP_B_ASGNED;
