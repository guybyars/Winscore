...
void CTask::Serialize(CArchive & ar)
	...
	if (ar.IsStoring())
	{
		...
		SerializeTime(ar, m_cGateOpenTime);
		ar << m_cPostTime;
		ar << CTimeSpan(0);
		ar << CTimeSpan(0);
		ar << CTimeSpan(0);
		...
		for( int i=0; i<m_nTurnpoints; i++) 
			{
			ar << m_aiTurnpointIDs[i];
			ar << m_afTurnpointRadius[i];
			}
		...
		ar << (WORD)m_b2ndGateActive;
		if( m_b2ndGateActive)
				m_cStartGate2.Serialize(ar);
		...
		if( iSchema<=3  )
			...
			for( int k=0; k<MAXTASKTPS+2; k++) ar >> aiTemp[k];

			if( m_nTurnpoints>=2 )
				{
				m_cStartGate.SetPointID( aiTemp[0] );
				m_cFinishGate.SetPointID( aiTemp[m_nTurnpoints-1] );
				m_nTurnpoints-=2;
				for(int  i=0; i<m_nTurnpoints; i++ )
					m_aiTurnpointIDs[i]=aiTemp[i+1];
				}
			else
				{
				m_cStartGate.SetPointID( 1 );
				m_cFinishGate.SetPointID( 1 );
				}
			...
			ar >> d;
			m_cStartGate.SetRadius(d);
			ar >> d;
			m_cFinishGate.SetRadius(d);

			ar >> w;
			m_cStartGate.SetFirstLegAtExit(((BOOL)w)?(true):(false));
			ar >> w;
			m_cFinishGate.SetFirstLegAtExit(((BOOL)w)?(true):(false));

			ar >> dDum;

			if( iSchema==1 )
				{	
				m_cStartGate.SetHeight(5000);
				m_cFinishGate.SetBase(200);
				}
			if( iSchema>=2  )
				{
				ar >> i;
				m_cStartGate.SetHeight(i);
				ar >> i;
				m_cFinishGate.SetBase(i);
				}
			if( iSchema==3  )
				{
				ar >> w;
				m_cFinishGate.SetGateType((EGateType)w);
				ar >> i;
				m_cFinishGate.SetHeading(i);
				ar >> w;
				m_cStartGate.SetGateType((EGateType)w);
				ar >> i;
				m_cStartGate.SetHeading(i);
				}
			}
		else if( iSchema==4 )
			{
			...
			ar >> m_nTurnpoints;
			for( int i=0; i<m_nTurnpoints; i++) ar >> m_aiTurnpointIDs[i];
			...
			ar >> w;
			(w==0)?(m_b2ndGateActive=false):(m_b2ndGateActive=true);
