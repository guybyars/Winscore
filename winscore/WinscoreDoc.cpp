...
//		m_straPassedLogs.GetXML(pIDOMWinscoreNode);
    ...
    /////////////////////////////////////////////////////////////////////////////
    // CWinscoreDoc serialization
    
    void CWinscoreDoc::Serialize(CArchive& ar)
    {
    	WORD w;
    	int iCurrentYear=CURRENTYEAR;
    	if (ar.IsStoring())
    		{
			ar << 8; // Write WinscoreDoc DB Version

    		ar<< iCurrentYear;
    
    		m_turnpointArray.Serialize(ar);
    		m_contestantList.Serialize(ar);
    		...
    		ar << (WORD)m_eUnits;
			ar << m_iSSA_ID;
    		ar <<  m_iNumContestDays;

    		SerializeTime(ar, m_CContestStartDate);
    		SerializeTime(ar, m_cPracticeDay1);
    		SerializeTime(ar, m_cPracticeDay2);

    		ar << (WORD)m_eCoordinateFormat;
    		ar << m_strImportFilePath;
    		ar << m_cTaskInfoString;
    
			int nClasses=GetNumClasses();
			ar<<nClasses;
    		for( int j=0; j<NUMCLASSES; j++ )
    			{
				CWSClass &cClass=GetClass(j);
				if( !cClass.IsActive() ) continue;
				ar << &cClass;
    			ar << (WORD)m_bGridPositionCalculated[j];
    			m_caGridPositions[j].Serialize(ar);
    			}
    
    		m_taskList.Serialize(ar);
    		m_ScoreRecordList.Serialize(ar);
    		m_PenalityList.Serialize(ar);
			m_FlightList.Serialize(ar);
			m_straPassedLogs.Serialize(ar);
			...
			if( iVersion<=2 )
    			...
    			ar >> strDum; //m_cContestDescription;
    			ar >> strDum; //m_cContestSerialNo;
    
    			ar >> w;	f18MeterClass=(BOOL)w;
    			ar >> w;  	fStandardClass=(BOOL)w;
    			ar >> w;  	f15MeterClass=(BOOL)w;
    			ar >> w;	fSports=(BOOL)w;
    			ar >> w;	fFAIClass=(BOOL)w;
    			ar >> w;	fOpen=(BOOL)w;

    			ar >> m_strContestName;
    			ar >> m_strLocation;
    			ar >> w;	m_eContest= (EContest)w;
    			ar >> w;	m_eUnits= (EUnits)w;
				if( m_eContest>1 ) m_eContest=eRegional;

				//Set the classes
				GetClass(m_eContest, e18Meter).SetActive(f18MeterClass?(true):(false));
				GetClass(m_eContest, eStandard).SetActive(fStandardClass?(true):(false));
				GetClass(m_eContest, e15Meter).SetActive(f15MeterClass?(true):(false));
				GetClass(m_eContest, eSports).SetActive(fSports?(true):(false));
				GetClass(m_eContest, eFAIHandicapped).SetActive(fFAIClass?(true):(false));
				GetClass(m_eContest, eOpen).SetActive(fOpen?(true):(false));

    			ar >>  m_iNumContestDays;
    			ar >>  m_CContestStartDate;
    			ar >>  m_cPracticeDay1;
    			ar >>  m_cPracticeDay2;
    			for( int i=0; i<m_iNumContestDays; i++) ar >>m_caContestDays[i];
    			ar >> w;	m_eCoordinateFormat=(ECoordFormat)w;
    			ar >> w;	// m_fEnforceSTI=(BOOL)w;
    			ar >> m_strImportFilePath;
    			ar >> m_cTaskInfoString;
    
    			for( int j=0; j<6; j++ )
    				{
    				ar >> w;   m_bGridPositionCalculated[j]=(BOOL)w;
    				m_caGridPositions[j].Serialize(ar);
    				}   
    			m_eventList.Serialize(ar);
    			m_taskList.Serialize(ar);
    			m_LandingCardList.Serialize(ar);
    			m_ScoreRecordList.Serialize(ar);
    			m_PenalityList.Serialize(ar);
    			m_InvalidLegList.Serialize(ar);
				if( iVersion!=1 )  // Don't read flights from version 1 files.
					m_FlightList.Serialize(ar);
				}
			else if( iVersion==3 || iVersion==4  )
				{
    			...
    			for( int j=0; j<nClasses; j++ )
    				{
					CWSClass *pClass=NULL;
					ar >> pClass;
					GetClass(pClass->m_eContest,pClass->m_eClass)=*pClass;
					delete pClass;
    				ar >> w;   m_bGridPositionCalculated[j]=(BOOL)w;
    				m_caGridPositions[j].Serialize(ar);
    				}
