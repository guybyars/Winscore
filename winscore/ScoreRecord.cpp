...
void CScoreRecord::Serialize(CArchive & ar)
{
	if (ar.IsStoring())
		{
		ar << m_cContestNo;
		...
		ar << (WORD)m_eClass;
		ar << (WORD)m_bGuest;
		ar << m_dAppliedPenalityPoints;
		ar << m_dPoints;
		ar << m_dSpeed;
		ar << m_dScoredSpeed;
		ar << m_dDistance;
		ar << m_dHandicappedSpeed;
		ar << m_dScoredHandicappedSpeed;
		ar << m_dHandicappedDistance;
		SerializeTime(ar, m_cStartTime);
		SerializeTime(ar, m_cFinishTime);
		ar << m_cSTOC;
		ar << (WORD)m_fFinisher;
		ar << (WORD)m_fContestant;
		ar << (WORD)m_uPenalityMask;
		ar << m_dCumPoints;
		ar << m_iCumRank;
		ar << m_iDayRank;
		ar << m_dWDSA;
		...
		if( iSchema<=2 ) 
			...
			m_fFinisher=(BOOL)w;
			ar >> w;
			m_fContestant=(BOOL)w;
			ar >> w;
			m_uPenalityMask=(int)w;
			if( iSchema==2 )
				{
				ar >> m_dCumPoints;
				ar >> m_iCumRank;
			...
			m_dScoredSpeed				=m_dSpeed;
			m_dScoredHandicappedSpeed	=m_dHandicappedSpeed;
			}
		else if( iSchema==3 )
			{
			WORD w;
