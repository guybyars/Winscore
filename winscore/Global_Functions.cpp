	...
	if( cFindFile.FindFile(strTemp) )
		...
		while( 1 )
			...
			if( rStatus.m_attribute & DIRECTORY ) 
				FindIGCFilesInDirectory( cDate, strFileName,  strArray );
			}
		}
		*/
	return;
...
bool CheckIGCSecurity( CString strIGCFile,  CString &strStatus )
	...
	if( cIGCFile.IsVolkslogger() )
	...
	else if( cIGCFile.IsCAI302File() )
		{
		char strResult[1024];
		memset(strResult,0,1024);
		int iRet=CAIValidateIGCFile( GetNonRelativePath(strIGCFile),
									1024,
									strResult);
		if( iRet==CER_OK )
			bSecure=true;
			...
			strStatus+=strIGCFile;
			strStatus+=_T("\r\n");
			strStatus+=CString(strResult);
			strStatus+=_T("\r\n");
			}

		}
  else if (cIGCFile.IsCAIGPSNAVFile() )
		{
		CString strTemp=GetNonRelativePath(strIGCFile);
		strTemp.MakeUpper();
		...
		if( CFile::GetStatus( strTemp, rStatus ) )
			...
			strStatus+=strIGCFile;
			strStatus+=_T("\r\n");
			strStatus+=_T("Could not find the  Could not find the .cai file generated by the GPS-NAV Recorder.");
			strStatus+=_T("\r\n");
			bSecure=false;
			}
	  }
...
CString PilotNameFromCAI(CString strCAIFile)
	...
	CString strItemText;
	ifstream	cIfstream;
	cIfstream.open(strCAIFile, ios::binary  );
	if (cIfstream.is_open() && !cIfstream.eof() )
		{
		char cRecord[40];
		cIfstream.getline( cRecord, 40 );
		cIfstream.close();
		int nCommas=0;
		for( int i=0; i<40; i++)
			{
			...
			if( nCommas<4 ) continue;
			if( nCommas>4 ) break;
			strItemText+=cRecord[i];
			}
