...
bool CZipArchive::ExtractFile(WORD uIndex, LPCTSTR lpszPath, DWORD nBufSize)
{
	...
	else
	{
		...
		DWORD dAtt=GetFileAttributes(szFile);
 		// Make sure extracted files aren't read only.

		if( dAtt & 0x01 )   
			{
			dAtt^=0x01;
			SetFileAttributes( szFile, dAtt );
			}
...
bool CZipArchive::AddNewFile(LPCTSTR lpszFilePath, LPCTSTR lpszArchiveName, int iLevel, unsigned long nBufSize)
{
	if (!nBufSize)
		return false;
...
//	header.m_szFileName = GetFileDirAndName(lpszFilePath);
	if( lpszArchiveName )
		header.m_szFileName=lpszArchiveName;
	else
		header.m_szFileName = GetFileName(lpszFilePath);
